...
function themeslug_enqueue_script()
{
    ...
    // wp_enqueue_script('stackup-core', plugin_dir_url(__FILE__) . 'assets/stackup.js', false);
    wp_enqueue_script('stackup', plugin_dir_url(__FILE__) . 'assets/st.js', false);
...
function add_to_card()
{
    ...
    $table = "wp_machin_order_data";
    $status = $wpdb->query("INSERT floatO $table (order_id, price, type_pcb, width, height, quantity, layers, type_order, dichte, dichte_done, distant, size_khoan, color, text_color, floaterface, test, note, file) VALUES ('" . $data['order_id'] . "', " . $data['price'] . ", '" . $data['type_pcb'] . "', '" . $data['width'] . "', '" . $data['height'] . "', '" . $data['quantity'] . "', '" . $data['layers'] . "', '" . $data['type_order'] . "', '" . $data['dichte'] . "', '" . $data['dichte_done'] . "', '" . $data['distant'] . "', '" . $data['size_khoan'] . "', '" . $data['color'] . "', '" . $data['text_color'] . "', '" . $data['floaterface'] . "', '" . $data['test'] . "', '" . $data['note'] . "', '" . $data['file'] . "')");
    if ($data['isStencil'] == 1) {
...
function calculate()
{
    ...
    if ($data['width'] <= 10 && $data['height'] <= 10 && $data['quantity'] == 5) {
        $gia_out = (float) get_option('duoi_10_sl5' . $lop);
    } else if ($data['width'] < 10 && $data['height'] < 10 && $data['quantity'] == 10) {
        $gia_out = (float) get_option('duoi_10_sl10' . $lop);
    } else {

        $pt1 = (float) ((($data['quantity'] - (float) get_option('so_luong_1' . $lop)[0]) * ((float) get_option('gia_1' . $lop)[1] - (float) get_option('gia_1' . $lop)[0])) / ((float) get_option('so_luong_1' . $lop)[1] - (float) get_option('so_luong_1' . $lop)[0])) + (float) get_option('gia_1' . $lop)[0];

        $pt2 = (float) ((($data['quantity'] - (float) get_option('so_luong_1' . $lop)[2]) * ((float) get_option('gia_1' . $lop)[3] - (float) get_option('gia_1' . $lop)[2])) / ((float) get_option('so_luong_1' . $lop)[3] - (float) get_option('so_luong_1' . $lop)[2])) + (float) get_option('gia_1' . $lop)[2];

        $gia_out =  (float) ((($data['width'] * $data['height'] - ((float) get_option('dai_1' . $lop)[0] * (float) get_option('rong_1' . $lop)[0])) * ($pt2 - $pt1)) / ((float) get_option('rong_1' . $lop)[2] * (float) get_option('dai_1' . $lop)[2] - (float) get_option('rong_1' . $lop)[0] * (float) get_option('dai_1' . $lop)[0])) + $pt1;
